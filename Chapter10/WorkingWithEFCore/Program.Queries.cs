...
using System.Security.Cryptography; // CollectionEntry
...
partial class Program
{
    static void QueryingCategories()
    {
        using (Northwind db = new())
        {
            ...
            //IQueryable<Category>? categories = db.Categories?.Include(c => c.Products); // Eager loading
            //IQueryable<Category>? categories = db.Categories; // Con lazy loading
            IQueryable<Category>? categories; // Explicit loading <-- el preferible (?
            ...
            else
            {
                ...
                foreach (Category c in categories)
                {
                    if (explicitLoading)
                    {
                        Write($"Explicit load products for {c.CategoryName}? (Y/N): ");
