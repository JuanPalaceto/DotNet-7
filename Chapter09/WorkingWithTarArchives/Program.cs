...
try
{
	...
	string tarFile = Path.Combine(current, "images-archives.tar");
	...
	if (File.Exists(tarFile))
	{
		...
		File.Delete(tarFile);
		WriteLine($"{tarFile} already existed so it was deleted.");
	}
	...
	{
		// If the destination directory does not exist then we must create it before extracting a Tar archive to it.
